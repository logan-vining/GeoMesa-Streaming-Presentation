...
public class PlaneDataStreamer {
    ...
    private static void runPlaneDemo() throws IOException {
        ...
        SimpleFeatureType planeSft = SimpleFeatureHelper.makeSftFromData("opensky_plane_data", new PlaneData());
        System.out.println(DataUtilities.encodeType(planeSft));
        prodStore.createSchema(planeSft);
        System.out.println("Created schema: " + DataUtilities.encodeType(planeSft));
        System.out.println();

        System.out.println("Starting stream...");
        System.out.println();

		StreamRunner streamRunner = new StreamRunner(prodStore, planeSft, new PlaneDataFetcher());
		Thread streamThread = new Thread(streamRunner);
		streamThread.start();
		
		System.out.println("Streaming OpenSky plane data; type 'stop' to stop and quit");
		System.out.println();
